...
// Helper: total IQD quantity across line items (matches "IQD" in description)
const getTotalIQDQty = (items = []) =>
  items.reduce((sum, it) => {
    const hasIQD =
      typeof it?.description === "string" &&
      /\bIQD\b/i.test(it.description);
    const qty = Number(it?.qty ?? 0);
    return sum + (hasIQD ? qty : 0);
  }, 0);
...
export async function buildInvoicePdfBuffer({
  ...
  iqdPurchased = 0, // kept for compatibility; auto-detected from items too
  subTotal = 0,
  shipping = 0,
  grandTotal = 0,
  currency = "AUD",
  ...
  try {
    ...
    doc.text(
      `${billTo.city || ""}${billTo.city ? ", " : ""}${billTo.region || ""} ${
        billTo.postcode || ""
      }`.trim(),
      L,
      billY + 24
    ...
    // ===== TABLE =====
    // Auto-append BONUS row if IQD quantity > 1,000,000 (or iqdPurchased > 1,000,000)
    const itemsWithBonus = Array.isArray(items) ? [...items] : [];
    const totalIQDQty = getTotalIQDQty(items);
    if (Number(iqdPurchased) > 1_000_000 || totalIQDQty > 1_000_000) {
      itemsWithBonus.push({
    ...
    itemsWithBonus.forEach((it, idx) => {
      ...
      const hasQty = it && (it.qty === 0 || it.qty || it.qty === "");
      const qtyStr =
        hasQty ? (it.qty === "" ? "" : String(it.qty)) : "1";
      const totalStr =
        it && (it.total === 0 || it.total || it.total === "")
      ...
      if (qtyStr !== undefined)
        doc.text(qtyStr, R - 45, y + 6, { align: "right" });
      if (totalStr !== undefined)
        doc.text(totalStr, R - 5, y + 6, { align: "right" });
    ...
    doc.text(`${subTotal} ${currency}`, valueX, totalsY + 12, {
      align: "right",
    });

    doc.text("Shipping Cost :", labelX, totalsY + 20);
    doc.text(`${shipping} ${currency}`, valueX, totalsY + 20, {
      align: "right",
    });
    ...
    doc.text(`${fmt2(grandTotal)} ${currency}`, valueX - 2, pillY + 7, {
      align: "right",
    });
    doc.setTextColor(0, 0, 0);
